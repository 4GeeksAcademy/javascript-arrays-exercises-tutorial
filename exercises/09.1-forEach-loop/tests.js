...
it("Please do not output ", function () {
    expect(app_content).not.toMatch(/The value of the item\(/);
});

it('Print only the numbers divisible by 2', function () {
    const _app = rewire('./app.js');
    const myArray = _app.__get__('myArray');
    let _output = "";
    myArray.forEach(function(item, index, arr){
        if(item % 2 === 0) _output += item + "\n";
    });
